    ...
    <tr id="vrow-<?php echo e($voucher->id); ?>"
        ...
        <td class="px-5 py-3.5">
            <p class="text-sm font-bold text-gray-900 dark:text-white">
                <?php echo e($voucher->formatted_value); ?> off
            </p>
            <?php if($voucher->min_order_amount > 0): ?>
            <p class="text-xs text-gray-400">Min £<?php echo e(number_format($voucher->min_order_amount,2)); ?></p>
            <?php endif; ?>
        </td>

        <td class="px-5 py-3.5 hidden sm:table-cell">
            <span class="text-sm text-gray-700 dark:text-gray-300">
                <?php echo e($voucher->assignedCustomer?->name ?? 'Any'); ?>
        ...
        <td class="px-5 py-3.5 hidden md:table-cell">
            ...
            <span class="text-sm <?php echo e($expired ? 'text-red-500 font-semibold' : 'text-gray-700 dark:text-gray-300'); ?>">
                <?php echo e(\Carbon\Carbon::parse($voucher->expiry_date)->format('d/m/Y')); ?>
        ...
        <td class="px-5 py-3.5 hidden md:table-cell">
            <span class="text-sm text-gray-700 dark:text-gray-300">
                <?php echo e($voucher->usage_limit ? $voucher->used_count . ' / ' . $voucher->usage_limit : $voucher->used_count); ?>
        ...
        <td class="px-5 py-3.5">
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full
                ...
                <?php if($active): ?><span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span><?php endif; ?>
        ...
        <td class="px-5 py-3.5" onclick="event.stopPropagation()">
            <div class="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button type="button" onclick="VoucherPage.selectVoucher(<?php echo e($data); ?>)"
                        title="Edit"
                ...
                <button type="button"
                        onclick="VoucherPage.printVoucher('<?php echo e(route('vouchers.print', $voucher)); ?>')"
                        title="Print"
                        class="w-7 h-7 rounded-lg flex items-center justify-center
                ...
                <button type="button"
                        onclick="VoucherPage.openEmailModal('<?php echo e(route('vouchers.email', $voucher)); ?>','<?php echo e(addslashes($voucher->code)); ?>','<?php echo e(addslashes($voucher->assignedCustomer?->email ?? '')); ?>','<?php echo e((bool)$voucher->assigned_to); ?>')"
                        title="Send Email"
                        class="w-7 h-7 rounded-lg flex items-center justify-center
                ...
                <button type="button"
                        onclick="VoucherPage.deleteVoucher(<?php echo e($voucher->id); ?>,'<?php echo e($voucher->code); ?>')"
                        title="Delete"
                        class="w-7 h-7 rounded-lg flex items-center justify-center
...
<?php endif; ?><?php /**PATH C:\Users\Your Tech Point\Desktop\Shop\phonesho_sas\resources\views/vouchers/partials/list.blade.php ENDPATH**/ ?>