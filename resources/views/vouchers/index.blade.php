...
<x-page-header
    ...
    :breadcrumbs="[['label'=>'Dashboard','route'=>'dashboard'],['label'=>'Vouchers']]">
</x-page-header>
...
<div class="flex items-center gap-2 flex-wrap mb-4">
    ...
    @endphp
    <button type="button"
            onclick="VoucherPage.filterStatus('{{ $val }}')"
            class="tab-btn inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-bold
                   border transition-all cursor-pointer {{ $cls }}"
            data-status="{{ $val }}">
        @if($active && $color === 'emerald')
        <span class="w-2 h-2 rounded-full bg-white"></span>
        @endif
        {{ $label }}
        <span class="text-xs {{ $active ? 'opacity-80' : 'text-gray-400' }}">{{ $count }}</span>
    </button>
    @endforeach
</div>
...
<div class="flex gap-5 items-start">
    ...
    <div class="flex-1 min-w-0">
        ...
        <div id="vlist-wrap"
            ...
            <div id="vlist-body">
                @include('vouchers.partials.list', compact('vouchers','summary'))
            </div>
        </div>
...
const VoucherPage = (() => {
    ...
    const baseUrl = '{{ route("vouchers.index") }}';
    const storeUrl= '{{ route("vouchers.store") }}';
    let suggested = '{{ $suggestedCode }}';
    let currentId = null;
    let statusFilter = '{{ request("status","") }}';
    let searchTimer;
    ...
    async function save() {
        ...
        try {
            const res  = await fetch(isEdit ? `{{ url('vouchers') }}/${currentId}` : storeUrl, {
    ...
    async function deleteVoucher(id, code) {
        ...
        try {
            const res  = await fetch(`{{ url('vouchers') }}/${id}`, {
