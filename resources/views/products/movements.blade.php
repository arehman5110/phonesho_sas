<x-app-layout>
<x-slot name="title">Stock Movements — {{ $product->name }}</x-slot>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td class="px-4 py-3.5">
                        <span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $typeColor }}">
                            {{ ucfirst($move->type) }}
                        </span>
                    </td>
                    <td class="px-4 py-3.5">
...
{{-- ── Top-up Modal (reused from index) ────────── --}}
<div id="topupModal"
        ...
        <div style="padding:24px;display:flex;flex-direction:column;gap:16px;">
            <div style="background:#f8fafc;border-radius:12px;padding:12px 16px;
                ...
                <span id="mv-current-stock"
                      ...
                      class="dark:text-white">{{ $product->stock }}</span>
            </div>
...
@push('scripts')
<script>
const _TOPUP_URL  = '{{ route("products.topup", $product) }}';
const _CSRF       = '{{ csrf_token() }}';
const _MOVE_URL   = '{{ route("products.movements", $product) }}';
